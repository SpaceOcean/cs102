...
def get_block(values: list, pos: tuple) -> list:
    ...
    for line in range(row, row + 3):
        for element in range(col, col + 3):
            values_of_square += values[line][element]

    return values_of_square
...
def check_solution(solution: list) -> bool:
    ...
    for row in range(9):
        for col in range(9):
            if set(get_row(solution, (row, col))) != set('123456789'):
                return False
            if set(get_col(solution, (row, col))) != set('123456789'):
                return False
